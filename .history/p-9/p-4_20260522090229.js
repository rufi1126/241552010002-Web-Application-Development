...
const rangking = [...withAvg].sort((a, b) => b.rata - a.rata);
rangking.forEach(m => console.log(`${m.nama}: ${m.rata.toFixed(1)}`));

//soal 4: grouping perjurusan
const perJurusan = data.redece((acc, m) => {
    if (!acc[m.jurusan]) acc[m.jurusan] = [];
    acc[m.jurusan].push(m);
    return acc;
}
